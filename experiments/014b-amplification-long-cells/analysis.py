...
# ///
"""
Experiment 014b analysis. Same per-method linear fits as 014,
plus:

1. Per-cell PWRCTRL/GPUPH classification (now usable because cells
   are long enough — most run 0.5-1 s, all back-to-back high-N cells
   run the full 5 s).
2. Slope-stability check vs 014's headline numbers.
3. Trial-level temporal join: for cells where p99/p50 > 1.5 (i.e.
   trial-level distribution is wide), bin trials by the contemporary
   PWRCTRL state and compare timing distributions. Specifically
   targets the b2b N=64/256/1024 collapse.
"""
...
# 014 stable-region slopes (M4 Max, AC, 50-trial cells, 2026-04-29
# timestamp prefix 20260429T115430). Used for the slope-stability
# check.
EXP014_INTERNAL_SLOPE = 405.28
EXP014_INTERNAL_INTERCEPT = 6860
EXP014_B2B_SLOPE = 3219.09
EXP014_B2B_INTERCEPT = 4711
...
def main():
    ...
    print()
    print("=" * 100)
    print("Slope-stability check: 014b vs 014")
    print("=" * 100)
    pre = [
        ("internal-loop", "slope", EXP014_INTERNAL_SLOPE,
         fits_stable["internal-loop"]["slope"] if fits_stable["internal-loop"] else None),
        ("internal-loop", "intercept", EXP014_INTERNAL_INTERCEPT,
         fits_stable["internal-loop"]["intercept"] if fits_stable["internal-loop"] else None),
        ("back-to-back", "slope", EXP014_B2B_SLOPE,
         fits_stable["back-to-back"]["slope"] if fits_stable["back-to-back"] else None),
        ("back-to-back", "intercept", EXP014_B2B_INTERCEPT,
         fits_stable["back-to-back"]["intercept"] if fits_stable["back-to-back"] else None),
    ]
    print(f"{'method':<14} {'metric':<10} {'014':>14} {'014b':>14} "
          f"{'Δ%':>8}  verdict")
    for method, metric, v014, v014b in pre:
        if v014b is None:
            print(f"{method:<14} {metric:<10} {v014:>14.2f} "
                  f"{'<n/a>':>14} {'-':>8}")
            continue
        delta_pct = (v014b - v014) / v014 * 100
        if abs(delta_pct) <= 5:
            verdict = "PASS"
            ...
            verdict = "FAIL"
        print(f"{method:<14} {metric:<10} {v014:>14.2f} "
              f"{v014b:>14.2f} {delta_pct:>+7.1f}%  {verdict}")
