...
# ///
"""
Experiment 014b: amplification + two-point timing under long cells.

Same kernels and N grid as 014, but each cell runs for ~5 s wall
clock (capped at MAX_TRIALS_PER_CELL trials) with 1 s idle gaps
between cells so IOReport's 250 ms cadence resolves per-cell DVFS
state. Sample-buffer resolve is deferred to the end of each cell
so CPU work doesn't perturb the steady state.

Each trial records its monotonic_ns so analysis can temporally
join trials to IOReport samples and ask whether the b2b N=1024
collapse correlates with within-cell DVFS state changes.
"""
...
# Metal caps a single counter sample buffer at 32 768 bytes
# (4 096 u64 slots = 2 048 trials × 2 timestamps). Resolve and reuse
# the buffer in chunks of this size; the resolve step is ~100 µs of
# memcpy + struct.unpack, small relative to per-chunk wall clock.
...
def resolve_range(sample_buffer, start_idx, count):
    """Bulk-resolve `count` consecutive timestamp slots starting at
    start_idx. Returns a list of `count` raw u64 values."""
...
def run_cell(method, n_amp, queue, pipeline, out_buffer, sample_buffer):
    """Run trials at sleep_0 in chunks of RESOLVE_CHUNK_TRIALS,
    resolving each chunk before starting the next so the sample
    buffer can be reused. Stops when cell_duration_s elapses or the
    per-cell trial cap is hit.

    Returns (cell_start_ns, cell_end_ns, full_trials) where
    full_trials is a list of dicts with monotonic_ns, cpu_total_ns,
...
def main() -> int:
    ...
    print("=" * 78)
    print("Experiment 014b: amplification under long cells")
    print("=" * 78)
    ...
    # One reusable sample buffer per cell, sized for one chunk; we
    # resolve and overwrite within the cell to fit Metal's 32 768 B
    # cap on counter sample buffers.
    sb_slots = 2 * RESOLVE_CHUNK_TRIALS
    print(f"sample-buffer slots per cell: {sb_slots} "
          f"({sb_slots * 8} B; resolves every {RESOLVE_CHUNK_TRIALS} "
          f"trials)")
    ...
    try:
        ...
        for method, n in plan:
            ...
            cell_idx += 1
            # Inter-cell idle so IOReport sees clean phase boundaries.
    ...
    meta_lines = [
        "experiment: 014b-amplification-long-cells",
        f"timestamp: {ts}",
    ...
    print()
    print(f"Now run: uv run experiments/014b-amplification-long-cells/analysis.py "
          f"--prefix {ts}")
    return 0
